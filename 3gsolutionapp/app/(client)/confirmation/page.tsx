...
// TICK-109 — Masquer bouton "Se connecter" si déjà connecté
// Redesign — card gradient + stepper + badge statut
...
import { CommandeStatusCard } from '@/components/client/CommandeStatusCard';
import { BackLink } from '@/components/ui';

// ─── Types ───────────────────────────────────────────────────────────────────

type StatutCommande = 'payee' | 'en_preparation' | 'prete' | 'recuperee';
...
function SuiviContent() {
  ...
  const estTerminee = data?.statut === 'prete' || data?.statut === 'recuperee';
    ...
    <>
      ...
      {!sessionId && (
        <div className="text-center py-16 space-y-2">
          ...
          <p className="text-red-500 text-lg font-medium">Lien invalide ou expiré</p>
          <p className="text-gray-500 text-sm">Aucun identifiant de commande trouvé.</p>
        </div>
      )}

      {/* ── Attente webhook ── */}
      {sessionId && (etat === 'loading' || etat === 'attente-webhook') && (
